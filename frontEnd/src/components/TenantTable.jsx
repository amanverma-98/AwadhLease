...
import { Calendar, ShieldCheck, ShieldAlert, PhoneCall, Trash2 } from 'lucide-react'
import { cn } from '../utils/cn'

export function TenantTable({ items, onRemove }) {
  return (
        ...
        <table className="w-full text-left text-sm border-collapse">
          ...
          <tbody className="divide-y divide-ink-100/50 dark:divide-ink-800/20">
            {items.map((tenant) => {
              ...
              const isInactive = String(tenant.status || '').toLowerCase() === 'inactive'
              return (
                <tr 
                  key={tenant.id} 
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {onRemove && (
                        <button
                          className={cn(
                            'p-1.5 rounded-lg border border-ink-100 dark:border-ink-800 transition shadow-sm',
                            isInactive
                              ? 'text-ink-300 dark:text-ink-600 cursor-not-allowed'
                              : 'text-rose-500 hover:bg-rose-50/60 dark:hover:bg-rose-950/30'
                          )}
                          onClick={() => onRemove(tenant)}
                          title={isInactive ? 'Tenant already removed' : 'Remove tenant'}
                          disabled={isInactive}
                          type="button"
                        >
