...
import { listTenants, createTenant, deleteTenant } from '../../services/tenantService'
...
export function TenantsPage() {
  ...
  const handleRemoveTenant = async () => {
    ...
    setRemoving(true)
    try {
      await deleteTenant(removeTarget.id)
      pushToast({
        title: 'Tenant removed',
        message: `${removeTarget.name} has been offboarded successfully.`
      })
      setRemoveTarget(null)
      load()
    } catch (error) {
      pushToast({ title: 'Removal failed', message: error.message })
    } finally {
      setRemoving(false)
    }
  }
  ...
  return (
    <div className="space-y-6 pb-20 animate-fade-in">
      ...
      <TenantTable items={tenants} onRemove={setRemoveTarget} />
      ...
      {removeTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          />
          <Card className="relative w-full max-w-md p-6 rounded-3xl border border-rose-200/60 dark:border-rose-900/50 bg-white dark:bg-ink-950 shadow-card">
            <h3 className="text-lg font-bold font-sora text-ink-950 dark:text-ink-50">
              Remove tenant
            </h3>
            <p className="mt-2 text-xs text-ink-500 dark:text-ink-400">
              This will deactivate {removeTarget.name} and preserve their history. This action
              can be reversed by an admin if needed.
            </p>
